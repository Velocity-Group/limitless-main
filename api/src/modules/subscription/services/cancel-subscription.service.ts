...
export class CancelSubscriptionService {
  ...
  public async cancelSubscription(id: string) {
    ...
    if (subscription.paymentGateway !== 'ccbill') {
      throw new Error('Not support cancel subscription for other payment gateways but ccbill');
    }
    ...
    const status = await CCBillService.cancelSubscription({
      subscriptionId: subscription.subscriptionId,
      ccbillClientAccNo: ''
    });
    if (!status) throw new Error(`Cannot unsubscribe subscription ${subscription.subscriptionId}`);
    subscription.status = SUBSCRIPTION_STATUS.DEACTIVATED;
    subscription.updatedAt = new Date();
    await subscription.save();
