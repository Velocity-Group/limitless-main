...
export default class TrendingProfiles extends PureComponent<any> {
  ...
  updateOrdering = async () => {
    ...
    await Promise.all([
      dataSource.forEach(async (data, index) => {
        await performerService.trendingUpdate({ performerId: data._id, ordering: index });
      })
    ]);
  ...
  async createNew() {
    ...
    try {
      await performerService.trendingCreate({ performerId: newProfileId, listType: 'subscription' });
  ...
  render() {
    ...
    return (
      <div>
        <Head>
          <title>Trending Profiles</title>
        </Head>
        <BreadcrumbComponent breadcrumbs={[{ title: 'Trending profiles' }]} />
