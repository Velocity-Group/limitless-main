            ...
            <div className="main-container">
              ...
              <div className="home-container">
                <div className="left-container">
                  {user._id && !user.verifiedEmail && ui.requireEmailVerification === 'true' && <Link href={user.isPerformer ? '/model/account' : '/user/account'}><a><Alert type="error" style={{ margin: '15px 0', textAlign: 'center' }} message="Please verify your email address, click here to update!" /></a></Link>}
                  <div className="visit-history">
                    ...
                    <div className="story-list">
                      {!loadingPerformer && randomPerformers.length > 0 && randomPerformers.map((per) => (
                        <Link href={{ pathname: '/model/profile', query: { username: per?.username || per?._id } }} as={`${per?.username || per?._id}`}>
